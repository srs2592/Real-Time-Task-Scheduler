...
public class edfEE {
	...
	int [] initialdeadlines = new int [5];
	...
	public edfEE(String[][] array){
		...
		for(int i=0;i<5;i++){
				...
				initialdeadlines[i] = deadlines[i];
		...
		while(p<5){
			...
			for(int i=0;i<3;i++){
				...
				else if(Utilization<1 && i==2){    //0.85 for sample2 text file with no deadines missed and if we can allow w3 to miss the first deadline then we can save more energy
		...
		///////////////////
		generateArrivals();	
		System.out.println("Energy Efficient EDF Scheduling Starts");
			...
			if (time==ArrivalQueue.peek()){
				...
				for(int iS=0;iS<S.length;iS++){
					...
					if(i1<arrivalsW1.length && S[iS]==0){
						if(time == arrivalsW1[i1]){
							...
							edfInput[0]=arrivalsW1[i1+1];//Integer.parseInt(dummyarray[1][1]);
							PreemptionQueue.add(arrivalsW1[i1+1]);
					...
					if(i2<arrivalsW2.length && S[iS]==1){
						if(time == arrivalsW2[i2]){
							...
							edfInput[1]=arrivalsW2[i2+1];//Integer.parseInt(dummyarray[2][1]);
							PreemptionQueue.add(arrivalsW2[i2+1]);
					...
					if(i3<arrivalsW3.length && S[iS]==2){
						if(time == arrivalsW3[i3]){
							...
							edfInput[2]=arrivalsW3[i3+1];//Integer.parseInt(dummyarray[3][1]);
							PreemptionQueue.add(arrivalsW3[i3+1]);
					...
					if(i4<arrivalsW4.length && S[iS]==3){
						if(time == arrivalsW4[i4]){
							...
							edfInput[3]=arrivalsW4[i4+1];//Integer.parseInt(dummyarray[4][1]);
							PreemptionQueue.add(arrivalsW4[i4+1]);
							//System.out.println(arrivalsW4[i4+1]+" W4 deadlines");
					...
					if(i5<arrivalsW5.length && S[iS]==4){
						if(time == arrivalsW5[i5]){
							...
							edfInput[4]=arrivalsW5[i5+1];//Integer.parseInt(dummyarray[5][1]);
							PreemptionQueue.add(arrivalsW5[i5+1]);
				...
				if(PreemptionQueue.isEmpty() && !ReadyQueue.isEmpty()){// 
					...
					if(execTime[m]>RunTime){
						...
						//edfInput[m]+=Integer.parseInt(dummyarray[m+1][1]);
					}
					time=time+RunTime;
				...
				/*
					...
					if(WaitingQueue.peek()<PreemptionQueue.peek())
						WaitingQueue.add(PreemptionQueue.poll());
				}*/
					...
					if(execTime[m]!=RunTime){
						//System.out.println(dead + " dead and corresponding task "+ (m+1)+ " And its execTime="+execTime[m]+" its Runtime="+RunTime);
						if(!WaitingQueue.contains(edfInput[m]))
							WaitingQueue.add(edfInput[m]);
						execTime[m]=execTime[m]-RunTime;
					}
	...
	public int getArrayIndex(int[] arr,int value) {
        ...
        for(int q=0;q<arr.length;q++){
            for(int j=q+1;j<arr.length;j++){
                 if(arr[q]==(arr[j])){
                	 if(k==q){
                		 if(initialdeadlines[k]<initialdeadlines[j]){
                			 edfInput[k]=edfInput[k]+1;
                		 }
                		 else{
                			 k=j;
                			 edfInput[j]=edfInput[j]+1;
                		 }
	...
	public void generateArrivals(){
		...
		}while(arrivalsW1[u]<1200);
		
		u=-1;
		do{
			u++;
			arrivalsW2[u]=(u+1)*edfInput[1];
		}while(arrivalsW2[u]<1200);
		
		u=-1;
		do{
			u++;
			arrivalsW3[u]=(u+1)*edfInput[2];
		}while(arrivalsW3[u]<1200);
		
		u=-1;
		do{
			u++;
			arrivalsW4[u]=(u+1)*edfInput[3];
		}while(arrivalsW4[u]<1200);
		
		u=-1;
		do{
			u++;
			arrivalsW5[u]=(u+1)*edfInput[4];
		}while(arrivalsW5[u]<1200);
